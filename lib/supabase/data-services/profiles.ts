 ...
 */
export async function getProfileOnboardingStatus(id: string): Promise<{
  data: { is_onboarded: boolean } | null
  error: unknown
}> {
  return await supabase.from('profiles').select('is_onboarded').eq('id', id).single()
}
 ...
 */
export async function createProfile(id: string, name: string): Promise<{ error: unknown }> {
  const { error } = await supabase.from('profiles').insert([{ id, name, is_onboarded: false }])
...
export async function updateProfileOnboardingStatus(
  ...
  const { error } = await supabase.from('profiles').update({ is_onboarded }).eq('id', id)
